...
function App() {
  ...
  useEffect(() => {
    ...
    const setupCloudSync = async () => {
      try {
        // Проверяем флаг, чтобы не загружать данные повторно после перезагрузки
        const syncFlag = sessionStorage.getItem(`sync-loaded-${user.id}`);
        ...
        try {
          ...
          // Загружаем данные только если еще не загружали в этой сессии
          if (!syncFlag) {
            try {
        ...
        if (!syncFlag) {
          ...
          if (hasCloudData) {
            ...
            if (success) {
              ...
              sessionStorage.setItem(`sync-loaded-${user.id}`, 'true');
